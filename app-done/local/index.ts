...
class LocalExecutionApp {
  ...
  identifyHandler(request: IntentFlow.IdentifyRequest):
    ...
    console.log(`IDENTIFY handler: received local device id ${localDeviceId}`);

    if (!localDeviceId.toString().match(/^deviceid[0-9]{3}$/gi)) {
      const err = new IntentFlow.HandlerError(request.requestId, 'invalid_device', 'Invalid device id from scan data: ' + localDeviceId);
      return Promise.reject(err);
    }
